...
var _FMwebhookUrl = '';
var _FMuserAgent = 'FMWEB Sample Webhook 0.2';
var _FMmaxTTL = null;
...
module.exports.init = async function (options,cb) {
  if(options) {
...
async function submitWebhook(payloadData) {
...
module.exports.submit = async function (payload) {
  if(_FMwebhookUrl.length<20 || !_FMwebhookUrl.startsWith('http')) throw(new Error('webhookUrl not configured'));
  ...
  if(!assets && !vulns) throw(new Error('No assets or vulnerabilities'));
  if(assets) {
    for(var i=0,l=payload.assets.length; i<l; i++) {
      let err=assetVerification(payload.assets[i]);
      if(err != null) throw(err);
    }
  }
  if(vulns) {
    for(var i=0,l=payload.vulnerabilities.length; i<l; i++) {
      let err=vulnerabilityVerification(payload.vulnerabilities[i]);
      if(err != null) throw(err);
    }
  }
  return submitWebhook(payload);
}
